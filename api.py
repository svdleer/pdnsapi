...
def api_get_zones(server_id):
    current_app.logger.debug("Getting zones for server: {}".format(server_id))
    
    if server_id == 'pdnsadmin':
        # Return zones from local database
        if g.apikey.role.name not in ['Administrator', 'Operator']:
            domain_obj_list = g.apikey.domains
        else:
            domain_obj_list = Domain.query.all()
        
        current_app.logger.debug("Found {} zones in database".format(len(domain_obj_list)))
        return jsonify(domain_schema.dump(domain_obj_list)), 200
    else:
        # Forward to PowerDNS API
        resp = helper.forward_request()
        
        if resp.status_code != 200:
            current_app.logger.error("PowerDNS API returned status: {}".format(resp.status_code))
            return resp.content, resp.status_code, resp.headers.items()
            
        # Filter zones for non-admin users
        if g.apikey.role.name not in ['Administrator', 'Operator']:
            try:
                # Get allowed domains for this API key
                domain_list = [d['name'] for d in domain_schema.dump(g.apikey.domains)]
                accounts_domains = [d.name for a in g.apikey.accounts for d in a.domains]
                allowed_domains = set(domain_list + accounts_domains)
                
                current_app.logger.debug("Allowed domains for user: {}".format(len(allowed_domains)))
                
                # Filter PowerDNS API response
                api_zones = json.loads(resp.content)
                filtered_zones = [z for z in api_zones if z['name'].rstrip('.') in allowed_domains]
                
                current_app.logger.debug("Filtered {} zones from {} total".format(
                    len(filtered_zones), len(api_zones)))
                    
                return json.dumps(filtered_zones), resp.status_code, resp.headers.items()
            except Exception as e:
                current_app.logger.error("Error filtering zones: {}".format(e))
                return resp.content, resp.status_code, resp.headers.items()
        else:
            # Admin users get all zones
            return resp.content, resp.status_code, resp.headers.items()
...
def api_get_domains():
    """
    Get all domains - Administrator role only
    """
    try:
        # Get domains from PowerDNS API
        pdns_api_url = Setting().get('pdns_api_url')
        pdns_api_key = Setting().get('pdns_api_key')
        pdns_version = Setting().get('pdns_version')
        api_uri_with_prefix = utils.pdns_api_extended_uri(pdns_version)
        ...
        current_app.logger.debug("Fetching domains from PowerDNS API: {}".format(api_full_uri))
        ...
        if resp.status_code == 200:
            current_app.logger.debug("Successfully fetched domains from PowerDNS API")
            return resp.content, resp.status_code, resp.headers.items()
        else:
            current_app.logger.error("PowerDNS API returned status: {}".format(resp.status_code))
            return make_response(jsonify({
                'status': 'error',
                'msg': 'Failed to fetch domains from PowerDNS API'
            }), resp.status_code)
            
    except Exception as e:
        current_app.logger.error("Error fetching domains: {}".format(e))
        return make_response(jsonify({
            'status': 'error',
            'msg': 'Internal server error while fetching domains'
        }), 500)
...
def api_create_domain():
    """
    Create a new domain - Administrator role only
    """
    try:
        data = request.get_json(force=True)
        ...
        current_app.logger.debug("Creating domain via PowerDNS API: {}".format(data))
        ...
        if resp.status_code == 201:
            current_app.logger.debug("Domain created successfully")
            
            # Update local domain database
            domain = Domain()
            domain.update()
            
            # Add history
            domain_id = domain.get_id_by_name(data['name'].rstrip('.'))
            history = History(msg='Create domain {0} via API'.format(data['name'].rstrip('.')),
                              detail=json.dumps(data),
                              created_by=is_custom_header_api(),
                              domain_id=domain_id)
            history.add()
            
            return resp.content, resp.status_code, resp.headers.items()
        else:
            current_app.logger.error("Failed to create domain. PowerDNS API status: {}".format(resp.status_code))
            return resp.content, resp.status_code, resp.headers.items()
            
    except Exception as e:
        current_app.logger.error("Error creating domain: {}".format(e))
        return make_response(jsonify({
            'status': 'error',
            'msg': 'Internal server error while creating domain'
        }), 500)


@api_bp.route('/domains/<string:domain_name>', methods=['GET'])
@apikey_auth
@apikey_is_admin
def api_get_domain(domain_name):
    """
    Get specific domain details - Administrator role only
    """
    try:
        pdns_api_url = Setting().get('pdns_api_url')
        pdns_api_key = Setting().get('pdns_api_key')
        ...
        current_app.logger.debug("Fetching domain {} from PowerDNS API".format(domain_name))
        ...
        if resp.status_code == 200:
            current_app.logger.debug("Successfully fetched domain {}".format(domain_name))
            return resp.content, resp.status_code, resp.headers.items()
        else:
            current_app.logger.error("PowerDNS API returned status: {} for domain {}".format(resp.status_code, domain_name))
            return resp.content, resp.status_code, resp.headers.items()
            
    except Exception as e:
        current_app.logger.error("Error fetching domain {}: {}".format(domain_name, e))
        return make_response(jsonify({
            'status': 'error',
            'msg': 'Internal server error while fetching domain'
        }), 500)


@api_bp.route('/domains/<string:domain_name>', methods=['PUT'])
@apikey_auth
@apikey_is_admin
@csrf.exempt
def api_update_domain(domain_name):
    """
    Update domain (modify records, settings, etc.) - Administrator role only
    """
    try:
        data = request.get_json(force=True)
        ...
        current_app.logger.debug("Updating domain {} via PowerDNS API".format(domain_name))
        ...
        if resp.status_code == 204:
            current_app.logger.debug("Domain {} updated successfully".format(domain_name))
            
            # Add history
            domain = Domain()
            domain_id = domain.get_id_by_name(domain_name.rstrip('.'))
            history = History(msg='Update domain {0} via API'.format(domain_name.rstrip('.')),
                              detail=json.dumps(data),
                              created_by=is_custom_header_api(),
                              domain_id=domain_id)
            history.add()
            ...
            current_app.logger.error("Failed to update domain {}. PowerDNS API status: {}".format(domain_name, resp.status_code))
            return resp.content, resp.status_code, resp.headers.items()
            
    except Exception as e:
        current_app.logger.error("Error updating domain {}: {}".format(domain_name, e))
        return make_response(jsonify({
            'status': 'error',
            'msg': 'Internal server error while updating domain'
        }), 500)


@api_bp.route('/domains/<string:domain_name>', methods=['DELETE'])
@apikey_auth
@apikey_is_admin
@csrf.exempt
def api_delete_domain(domain_name):
    """
    Delete domain - Administrator role only
    """
    try:
        pdns_api_url = Setting().get('pdns_api_url')
        pdns_api_key = Setting().get('pdns_api_key')
        ...
        current_app.logger.debug("Deleting domain {} via PowerDNS API".format(domain_name))
        ...
        if resp.status_code == 204:
            current_app.logger.debug("Domain {} deleted successfully".format(domain_name))
            ...
            # Add history
            history = History(msg='Delete domain {0} via API'.format(domain_name.rstrip('.')),
                              detail='',
                              created_by=is_custom_header_api(),
                              domain_id=domain_id)
            history.add()
            ...
            current_app.logger.error("Failed to delete domain {}. PowerDNS API status: {}".format(domain_name, resp.status_code))
            return resp.content, resp.status_code, resp.headers.items()
            
    except Exception as e:
        current_app.logger.error("Error deleting domain {}: {}".format(domain_name, e))
        return make_response(jsonify({
            'status': 'error',
            'msg': 'Internal server error while deleting domain'
        }), 500)


@api_bp.route('/domains/<string:domain_name>/records', methods=['GET'])
@apikey_auth
@apikey_is_admin
def api_get_domain_records(domain_name):
    """
    Get domain records - Administrator role only
    """
    try:
        pdns_api_url = Setting().get('pdns_api_url')
        pdns_api_key = Setting().get('pdns_api_key')
        ...
        current_app.logger.debug("Fetching records for domain {} from PowerDNS API".format(domain_name))
        ...
        if resp.status_code == 200:
            current_app.logger.debug("Successfully fetched records for domain {}".format(domain_name))
            # Extract just the rrsets from the response
            domain_data = json.loads(resp.content)
            if 'rrsets' in domain_data:
                return json.dumps(domain_data['rrsets']), resp.status_code, resp.headers.items()
            else:
                return resp.content, resp.status_code, resp.headers.items()
        else:
            current_app.logger.error("PowerDNS API returned status: {} for domain {} records".format(resp.status_code, domain_name))
            return resp.content, resp.status_code, resp.headers.items()
            
    except Exception as e:
        current_app.logger.error("Error fetching records for domain {}: {}".format(domain_name, e))
        return make_response(jsonify({
            'status': 'error',
            'msg': 'Internal server error while fetching domain records'
        }), 500)
